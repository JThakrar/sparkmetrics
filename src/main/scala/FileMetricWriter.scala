...
/**
  ...
  * depending upon whether the HADOOP_CONF_DIR environment variable.
  * If the environment variable is set and has appropriate hadoop conf files,
  * then the output goes to files on HDFS. Else it goes to local filesystem.
  * The filenames have the format "<metric_type>_<id>.json" where
  * metric_type = app or job or stage
